...
may <- data.frame(xmin=1, xmax=3.214, ymin=0.0001, ymax=Inf, label = "May") 
jun <- data.frame(xmin=3.214, xmax=5.357, ymin=0.0001, ymax=Inf, label = "Jun")
jul <- data.frame(xmin=5.357, xmax=7.571, ymin=0.0001, ymax=Inf, label = "Jul")
aug <- data.frame(xmin=7.571, xmax=9.786, ymin=0.0001, ymax=Inf, label = "Aug")
sep <- data.frame(xmin=9.786, xmax=11.929, ymin=0.0001, ymax=Inf, label = "Sep")
oct <- data.frame(xmin=11.929, xmax=Inf, ymin=0.0001, ymax=Inf, label = "Oct")
...
fit <- glmmTMB(formula = succ ~  period+year+lat+lat*period+(1|place) + (1|mic),  # Fit glmmTMB-model
               offset = log(weight), data = test, family = nbinom2)
...
ggplot(data = filter(eff), aes(period+lat_offset*0.1-0.3, estimate)) +
  ...
  scale_y_continuous(name = "Activity",breaks = c(1,10,100), trans = "log") +
  ...
  geom_text(data = may, aes(label = label, x=xmin+0.6, y = 280), color = "grey40") +
  geom_text(data = jun, aes(label = label,x=xmin+0.6, y = 280), color = "grey40") +
  geom_text(data = jul,aes(label = label,x=xmin+0.6, y = 280), color = "grey40") +
  geom_text(data = aug, aes(label = label,x=xmin+0.6, y = 280), color = "grey40") +
  geom_text(data = sep, aes(label = label,x=xmin+0.6, y = 280), color = "grey40") +
  geom_text(data = oct, aes(label = label,x=xmin+0.3, y = 280), color = "grey40") +
  coord_cartesian(xlim = c(1, 12), ylim = c(0.7, 250))
...
fit <- glmmTMB(formula = succ ~  period+year+lat+lat*period+(1|station/place) + (1|mic), # Fit glmmTMB-model
               offset = log(weight), data = test, family = nbinom2)
summary(fit) # Print model summary
summary(effall <- allEffects(fit, se=list(level=0.95))) # Print model estimates
...
ggplot(data = filter(eff), aes(period+lat_offset*0.1-0.3, estimate)) +
  ...
  geom_errorbar(aes(ymax=upper, ymin=lower, colour = lat), size = 0.75, width=.1) +
  scale_x_continuous(name = "Period", breaks = seq(1,12,1)) +
  scale_y_continuous(name = "Activity", breaks = c(0.001, 0.01,0.1,1,10,100,1000), labels = c("0.001", "0.01", "0.1", "1", "10", "100", "1000"), trans = "log") +
  scale_colour_manual(values = c("#d59c00", "#ffcd41", "#ffe68b", "#d583cd", "#83416a"), name = "Latitude °N") + 
  theme_minimal(base_line_size = 0) + 
  theme(panel.grid.minor.x = element_line(color = "grey85", size = 1, linetype= "solid"),
        panel.grid.major.x = element_line(color = "black", size = 1, linetype = "blank"),
  ...
  geom_text(data = may, aes(label = label, x=xmin+0.6, y = 3500), color = "grey40") +
  geom_text(data = jun, aes(label = label,x=xmin+0.6, y = 3500), color = "grey40") +
  geom_text(data = jul,aes(label = label,x=xmin+0.6, y = 3500), color = "grey40") +
  geom_text(data = aug, aes(label = label,x=xmin+0.6, y = 3500), color = "grey40") +
  geom_text(data = sep, aes(label = label,x=xmin+0.6, y = 3500), color = "grey40") +
  geom_text(data = oct, aes(label = label,x=xmin+0.3, y = 3500), color = "grey40") +
  coord_cartesian(xlim = c(1, 12), ylim = c(0.0005, 3000))



###### P. NATHUSII ########

sub_data <- filter(df, taxa == "P. nathusii", station == "Huso" | station == "Tvarminne" | station == "Seili") # Subset data with one taxa
fit <- glmmTMB(formula = succ ~ period+year+ period*year  + (1|station/place) + (1|mic), # Fit glmmTMB model
               offset = log(weight), data = test, family = nbinom2)
fit2 <- update(fit, control=glmmTMBControl(optCtrl=list(iter.max=1e3,eval.max=1e3))) # Add iterations to reach convergence
summary(fit) # Print model summary
summary(effall <- allEffects(fit, se=list(level=0.95))) # Print model estimates
...
ggplot(data = filter(eff), aes(period+year_offset*0.1-0.3, estimate)) +
  ...
  scale_x_continuous(name = "Period", breaks = seq(1,12,1)) +
  scale_y_continuous(name = "Activity", breaks = c(0.1,1,10,100), trans = "log") +
  ...
  theme(panel.grid.minor.x = element_line(color = "grey85", size = 1, linetype= "solid"),
        ...
        panel.grid.major.y = element_line(color = "grey85", size = 1, linetype = "dotted"),
        legend.position = "bottom") +
  geom_text(data = may, aes(label = label, x=xmin+0.6, y = 400), color = "grey40") +
  geom_text(data = jun, aes(label = label,x=xmin+0.6, y = 400), color = "grey40") +
  geom_text(data = jul,aes(label = label,x=xmin+0.6, y = 400), color = "grey40") +
  geom_text(data = aug, aes(label = label,x=xmin+0.6, y = 400), color = "grey40") +
  geom_text(data = sep, aes(label = label,x=xmin+0.6, y = 400), color = "grey40") +
  geom_text(data = oct, aes(label = label,x=xmin+0.3, y = 400), color = "grey40") +
  coord_cartesian(xlim = c(1, 12), ylim = c(0.03, 350))
